...
int n, m, root, nown, nowi, tot, nowsplit, target[2], vis[maxn], used[maxn];
double range;
stack<int> stk;
int cmps(double t)
{
...
double calc(int split, int d)
{ 
	if (split == 0) return d / 10000000.0; else return d / 1000000.0;
}
void findnearest(FILE *fout)
{
	...
	while (!stk.empty())
	{
		...
		{
			if (used[bp] != nowi && cmps(dist(kd[bp].d, target) - range) <= 0)
			{
				used[bp] = nowi;
		...
		else
		{
			...
			if (target[s] <= kd[bp].d[s])
			{
				...
				if (cmps(abs(calc(s,kd[bp].d[s] - target[s])) - range) < 0)
				{
					psearch = kd[bp].rc;
					if (psearch && vis[psearch] != nowi) stk.push(psearch);
				}
...
int main()
{
	...
	while (ch == 'y' || ch == 'Y')
	{
		...
		for (int i = 1; i <= m; i++)
		{
			nowi = i;
			nown = 0;
			fscanf(fin, "%lf %d %d", &range, &target[0], &target[1]);
			findnearest(fout);
		}
		fclose(fin);
		fclose(fout);
